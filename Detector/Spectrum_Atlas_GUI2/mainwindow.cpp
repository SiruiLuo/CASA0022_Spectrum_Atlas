    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(&loggerTimer_, &QTimer::timeout, this, [this]{
        ...
        if (f.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            ui->loggerStatusLabel->setText(s.isEmpty() ? "Scan Status: --" :
                                                     "Scan Status: " + s);
        }
    });
    loggerTimer_.start();

    ui->mapLabel->setText(tr("Waiting for map..."));
...
{
    if (roscoreProc_) {
        QMessageBox::information(this, tr("Info"), tr("ROS is already running."));
        return;
    }
    ...
    if (!roscoreProc_->waitForStarted(3000)) {
        QMessageBox::critical(this, tr("Error"), tr("Failed to start roscore!"));
...
void MainWindow::killProcesses()
{
    ...
    ui->loggerStatusLabel->setText("Scan Status: --");
    ui->mapLabel->setText(tr("Waiting for map..."));
}
...
{
    if (QMessageBox::question(this, tr("Exit Confirmation"),
                              tr("Are you sure you want to exit and shut down all ROS nodes?"))
        == QMessageBox::Yes)
    {
